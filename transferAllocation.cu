...
{ // type = { cu_loadFromDeviceToHost, cu_loadFromHostToDevice }
    ...
    logging::FileLogger* Log = gpu->Log;
    if(type == cu_loadFromDeviceToHost) {
        HANDLE_CUERROR( cudaMemcpyAsync( host, gpu->m_borders,
            size * sizeof(Cell), cudaMemcpyDeviceToHost, gpu->streamHaloBorder ) );
        HANDLE_CUERROR( cudaMemcpyAsync( host + size,
            gpu->m_borders + size,
            size * sizeof(Cell), cudaMemcpyDeviceToHost, gpu->streamHaloBorder ) );
        ...
        *Log << "Stream 'streamHaloBorder' loaded border data from device to host and synchronized correctly.";
        ...
        HANDLE_CUERROR( cudaMemcpyAsync( gpu->m_borders, host,
            size * sizeof(Cell), cudaMemcpyHostToDevice, gpu->streamHaloBorder ) );
        HANDLE_CUERROR( cudaMemcpyAsync( gpu->m_borders + size, host + size,
            size * sizeof(Cell), cudaMemcpyHostToDevice, gpu->streamHaloBorder ) );
        cudaStreamSynchronize(gpu->streamHaloBorder);
        *Log << "Stream 'streamHaloBorder' loaded halo border from host to device and synchronized correctly.";
    } else {
...
extern "C" void cu_moveBorderDataToField(void* prop)
{
    cu_gpuProperties* gpu = (cu_gpuProperties*) prop;
    logging::FileLogger* Log = gpu->Log;
    int size = gpu->m_bNy - 2;
    HANDLE_CUERROR( cudaMemcpyAsync( gpu->m_Field + (size + 2) + 1, gpu->m_borders,
        size * sizeof(Cell), cudaMemcpyDeviceToDevice, gpu->streamHaloBorder ) );
    HANDLE_CUERROR( cudaMemcpyAsync( gpu->m_Field + gpu->m_Field_size - 2*(size + 2) + 1, gpu->m_borders + size,
        size * sizeof(Cell), cudaMemcpyDeviceToDevice, gpu->streamHaloBorder ) );
    cudaStreamSynchronize(gpu->streamHaloBorder);
    *Log << "Stream 'streamHaloBorder' moved border from m_borders to m_Field and synchronized correctly.";
}
